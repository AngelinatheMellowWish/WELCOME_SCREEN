...
> Version history: registered in CHANGELOG[v1.2.0].md
> English edition of `MANUAL[v1.2.0].md` (bilingual delivery, maintained in sync)
...
> Save runs validation → atomic disk write → main process hot-reload broadcast; the rule editor footer also has **Save / Apply** buttons. **Closing the window** with unsaved changes prompts Save / Discard / Cancel (**switching rules does not prompt** — edits stay in the rule list until you save); "Restore defaults" first confirms unsaved changes (AC-38).
...
| Program crash | Dump file in `log/crash/` (keeps last 10 or ≤200MB); logs in `log/`; report with [error codes](error_codes[v1.2.0].md) |
