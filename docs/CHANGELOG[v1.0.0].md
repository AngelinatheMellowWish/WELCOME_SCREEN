...
- 功能范围见 `docs/项目需求书[v1.2.0].md`（M1~M6 已实施；余 AC-24/42/74 实机验收与发布）
...
- 需求书升 **v1.2.0**（`项目需求书[v1.2.0].md`，重命名自 v1.1.0）：§3.2 NFR-02 与 §6 AC-67 空闲内存目标 **80MB→250MB（多进程合计）**——4 个 .NET/WPF 进程基线合计约 180MB，原 80MB 在 4 进程架构下不可达
