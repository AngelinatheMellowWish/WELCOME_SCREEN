...
> 文档版本：v1.2.0
> 状态：已基线确认（2026-09-09 十轮决策终审通过；v1.1.0 文档版本管理严格化；v1.2.0 于 2026-09-11 升版——v0.2.2 修复（切换弹窗/编辑区保存应用按钮/预览字号）级联登记）
> 日期：2026-09-08
...
> **2026-09-11 v0.2.2 修复级联升版**（切换弹窗 / 编辑区保存应用按钮 / 预览字号）：项目需求书 v1.4.0→**v1.5.0**；development_standards v1.3.0→**v1.4.0**；architecture / error_codes / CHANGELOG / AGENTS / VERSION_REGISTRY / README / MANUAL / MANUAL.en / TEST_REPORT v1.1.0→**v1.2.0**（CHANGELOG 自身亦升版；各分组详见下方对应表）。
...
| v1.4.0 | 2026-09-11 | **升版**（文档版本管理改革级联，次版本）：§7 文档清单交叉引用同步（development_standards v1.3.0 / architecture v1.1.0 / error_codes v1.1.0 / CHANGELOG v1.1.0 / VERSION_REGISTRY v1.1.0 / AGENTS v1.1.0 / README v1.1.0 / MANUAL v1.1.0 / MANUAL.en v1.1.0 / TEST_REPORT v1.1.0）；附录 A 决策记录追加「文档版本管理严格化」；文件名升版为 `项目需求书[v1.4.0].md` |

### 1.2 开发规范（development_standards[vX.Y.Z].md）

| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.3.0 | 2026-09-11 | **升版**（版本管理改革，次版本）：§1.6 文档版本管理改为**严格每次变更升版**——取消「指针/登记型不升版」豁免；新增升版粒度（每任务每文档一次）、CHANGELOG 同标准、工作记录按日不升版、级联说明；§1.6.1 程序集同标准（每次代码变更升、发布时对齐）；文件名升版为 `development_standards[v1.3.0].md` |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（版本管理改革级联，修订）：§7 配套文档引用同步（项目需求书 v1.4.0 / development_standards v1.3.0）；错误码文档引用同步 v1.1.0；文件名升版为 `architecture[v1.1.0].md` |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（版本管理改革级联，修订）：§3 登记流程中 architecture 引用同步 v1.1.0；文件名升版为 `error_codes[v1.1.0].md`（错误码总数不变，仍 49 条） |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（版本管理改革级联，修订）：文档索引表全部版本化文件名同步（需求书 v1.4.0 / development_standards v1.3.0 / architecture v1.1.0 / error_codes v1.1.0 / VERSION_REGISTRY v1.1.0 / AGENTS v1.1.0 / CHANGELOG v1.1.0 / MANUAL v1.1.0 / MANUAL.en v1.1.0 / TEST_REPORT v1.1.0）；文件名升版为 `README[v1.1.0].md` |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（AC-38 修复 + 版本管理级联，次版本）：§3 配置窗口指南补「关闭/切换规则时有未保存修改会提示 保存/放弃/取消（AC-38）」；CHANGELOG/error_codes 引用同步；文件名升版为 `MANUAL[v1.1.0].md` |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（AC-38 修复 + 版本管理级联，修订）：AC-38 备注更新为「已实现：关闭/切换未保存提示（保存/放弃/取消）」；需求书引用同步 v1.4.0；全量 396/396；文件名升版为 `TEST_REPORT[v1.1.0].md` |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（AC-38 修复 + 版本管理级联，次版本）：§3 Config Window Guide 补「closing/switching with unsaved changes prompts Save/Discard/Cancel (AC-38)」；CHANGELOG/error_codes 引用同步；文件名升版为 `MANUAL.en[v1.1.0].md` |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（版本管理改革 + 状态指针，次版本）：§1/§4 全部版本化文件名引用同步（需求书 v1.4.0 / development_standards v1.3.0 / architecture v1.1.0 / error_codes v1.1.0 / CHANGELOG v1.1.0 / VERSION_REGISTRY v1.1.0 / README v1.1.0 / MANUAL v1.1.0 / MANUAL.en v1.1.0 / TEST_REPORT v1.1.0）；§4 当前状态更新（F-78 + AC-38 修复 + 版本管理严格化 + 发布 v0.2.1）；文件名升版为 `AGENTS[v1.1.0].md`（根 AGENTS.md 稳定指针同步内容） |
...
| 版本 | 日期 | 变更摘要 |
|------|------|----------|
| v1.1.0 | 2026-09-11 | **升版**（版本管理改革，次版本）：§6.1 文档版本升版时机改为**严格每次内容变更升版**（取消豁免）；§6.1 程序集最低步进明确为「相关程序集每次代码变更升（修订/次/主）」；§1 当前值更新（程序集 0.2.1 / 发布 v0.2.1 / 文档版本）；§4 程序集表更新 0.2.0→0.2.1；§5 文档快照更新全部新版本；文件名升版为 `VERSION_REGISTRY[v1.1.0].md` |
...
### [v0.2.1] - 2026-09-11（发布：tag v0.2.1，含 F-78 + AC-38 修复 + 文档版本管理严格化）

#### Added（计划）
- 项目初始化：全部文档采用独立版本管理（头部版本 + 集中登记 + 文件名带版本号）
- 功能范围见 `docs/项目需求书[v1.5.0].md`（M1~M6 已实施，AC 99/99 通过）
...
#### Fixed（AC-38 未保存修改提示，2026-09-11）
- ConfigUI：`OnClosing`/`OnRuleSelectionChanged` 在 `_dirty` 时弹「保存/放弃/取消」（AC-38 本已要求，此前仅隐藏不提示 → 未保存规则静默丢失）；新增 `SaveToDisk`/`ConfirmUnsavedChanges`/`BroadcastConfig`；「放弃」经 `LoadConfig` 重载真正丢弃内存修改
- 根因：用户新建 Steam 规则后关闭配置窗口未保存 → Main 无该规则 → 不触发（已修复并随 v0.2.1 重发）

#### Changed（文档版本管理严格化，2026-09-11）
- development_standards §1.6：取消「指针/登记型不升版」豁免——任何文档内容增/删/改一律升版（每任务每文档一次）；CHANGELOG 同标准；工作记录按日不升版；新增级联说明
- VERSION_REGISTRY §6.1、CHANGELOG §3 同步；本轮全部 docs 版本级联升版；程序集 0.2.0→0.2.1；发布 v0.2.1

#### Released（v0.2.1，2026-09-11）
- tag **v0.2.1**（私有 Object1688 `develop` + 公开 WELCOME_SCREEN `main`）+ GitHub Release + `Object1688-v0.2.1-win-x64.zip`；程序集统一 0.2.1

#### Fixed（v0.2.2，2026-09-11）
- **AC-38 切换弹窗过频**：切换规则不再弹确认（编辑保存在各规则模型，切换不丢数据），仅关闭窗口时提示「保存/放弃/取消」——修复此前切换规则频繁弹窗、无法切换编辑的问题
- **编辑区缺保存/应用按钮**：规则编辑页底部新增「保存」「应用」按钮（此前仅顶部工具条有保存），两键均校验→原子写盘→ConfigChanged 热生效
- **预览字号上限**：ConfigUI 预览原固定 12–56 DIP，字号 >96 预览无变化；改为按预览区高度/行数自适应（上限 200 DIP）。真实渲染（Overlay）本无字号上限，`fontSize` 可任意放大（≥2×96=192 正常）

#### Released（v0.2.2，2026-09-11）
- tag **v0.2.2**（私有 Object1688 `develop` + 公开 WELCOME_SCREEN `main`）+ GitHub Release + `Object1688-v0.2.2-win-x64.zip`；程序集统一 0.2.2

---

## 3. 变更登记规则

- **升版触发（严格）**：任何 docs/ 文档内容的增/删/改 → 头部版本升号 → 重命名文件 → 在对应分组追加登记行。**无「指针/登记型不升版」豁免**（交叉引用更新、索引表增行、登记行本身等一律升版）
- **升版粒度**：每任务（一个逻辑变更）对同一文档升一次——任务内对同一文档的多次编辑合并为一次升版，改完再统一改名
- **登记内容**：版本号 / 日期 / 变更摘要（Added/Changed/Fixed/Breaking 要点）
- **CHANGELOG 自身**：与其它文档**同标准**——每次变更升版并重命名（如 `CHANGELOG[v1.0.0]` → `CHANGELOG[v1.1.0]`）
- **工作记录**：按日一份（`【YYYY-MM-DD】【开发工作记录】[vX.Y.Z].md`），当日续录追加**不升版、不重命名**
- **级联**：某文档升版导致其它文档交叉引用需同步时，被同步文档**同一任务内一并升版**（每文档每任务仅升一次，级联在任务内收敛）
- **.txt 同步**：任何 .md 升版后重跑 `build/sync-docs.ps1` 同步 .txt
